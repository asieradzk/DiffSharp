...
1. Make sure you have a CUDA-capable NVIDIA GPU with a recent driver installed. The TorchSharp CUDA packages include the CUDA runtime (currently CUDA 12.8, Windows and Linux).

2. Use `dotnet test /p:DIFFSHARP_TESTGPU=true`, or set the environment variable `DIFFSHARP_TESTGPU=true`

3. Verify that `dsharp.isCudaAvailable()` returns true. The tests run on every available device.
